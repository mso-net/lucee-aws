<cfscript>
	...
	url.cpu = url.cpu ?: false;

	rootMapping = '/tests';
	rootPath = ( DirectoryExists( rootMapping ) ) ? rootMapping : ExpandPath( rootMapping );
	...
	switch ( url.action ?: '' ) {
		case 'runTestBox':
			...
			if (
						...
						.init(
							directory={
								...
								'filter': function( 
									...
									return ( 
										ListLast( arguments.path , '/' ) != 'Application.cfc' 
									);
								} 
							}
						)
				...
				WriteOutput(
					'<h1>Invalid incoming directory: ' & rootMapping & url.path & '</h1>'
				);
			}
	...
	directory
		action = 'list'
		directory = rootPath & url.path
		name = 'qResults'
		sort = 'asc';

	executePath = rootMapping & url.path;
	...
	if (
		...
		backPath = url.path
			.ReplaceNoCase( url.path.ListLast( '/' ) , '' )
			.ReReplace( '/$', '' );
	}
</cfscript>
